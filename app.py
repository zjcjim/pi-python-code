    ...
    if is_target_lost:
        ...
        if abs(previous_angle_x - 90) > 60:
            ...
            if previous_angle_x > 90:
                # turn left
                # add a coefficent
                motor_speed_smoothing([30, speed_diff * 1.2 + 150, speed_diff * 1.2 + 150, 30], 45)
            else:
                # turn right
                motor_speed_smoothing([speed_diff + 150, 40, 40, speed_diff + 150], 45)
        else:
            # go straight
            motor_speed_smoothing([120, 120, 120, 120], 60)
...
def position_event():
    ...
    if position_x is not None and position_y is not None:
        ...
        if not is_target_destroyed:
            ...
            if target_lost_counter < 8 and is_target_lost == False:
